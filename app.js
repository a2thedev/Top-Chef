...
function searchCuisines(allDishes) {
    ...
    let userChoice = customPrompt("Please enter a cuisine: Mexican, Italian, Hungarian, French, Vegetarian or Irish?");
    let results = allDishes.filter(function(el){
        if(el.cuisine.includes(userChoice)){
            return true;
        } else{
            return false;
        }
...
function searchIngredients(allDishes) {
    ...
    let userChoice = customPrompt("Please enter an ingredient: tomato, beef, chickpea, corn, cheese, cabbage, flour, or sugar?");
    let results = allDishes.filter(function(el){
...
function runApp(allDishes, specialDish) {
    ...
    switch (userChoice) {
        ...
        case "5":
            ...
            break
        case "6":
            let texts = textMessage(specialDish)
            console.log(texts)
            ...
            break
        case "7":
            let emails = emailMessage(specialDish)
            console.log(emails)
